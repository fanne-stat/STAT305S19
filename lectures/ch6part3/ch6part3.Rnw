...
\pause \begin{center}
<<echo=F, out.width = "0.6\\textwidth", out.height = "0.6\\textheight", small.mar = T>>=
curve(dt(x, df = 9), from = -4, to = 4, ylim = c(0, .45))
abline(v = qt(0.95, df = 9))
...
shade_under_curve(function(x) dt(x,df = 9), qt(p=0.95, df = 9), 4)
text(x = 2.6, y = .3, labels = "t_9,0.95", cex = 1.5)
legend("topright", cex = 1.25, fill = "blue", legend = "Integral over rejection region = alpha")
@
...
text(labels = "z = 2.5", x = 3, y = .05, cex = 1.2)
