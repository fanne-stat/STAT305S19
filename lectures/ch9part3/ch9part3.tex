...
\end{align*}
\uncover<3->{\item With $ |t| = 1.089 < 2.16$, we fail to reject $H_0$.}
...
\uncover<4->{Var(\wh{\mu}_{y \mid \vc{x}})} & \uncover<4->{= \sigma^2 A(\vc{x})^2}
\end{align*}
\uncover<4->{for some constant $A(\vc{x})$. (different for different $\vc{x}$)}
\uncover<5->{\item Under the model assumptions:}
\begin{align*}
\uncover<6->{Z = \frac{\wh{\mu}_{y \mid \vc{x}} - \mu_{y \mid \vc{x}}}{\sigma A(\vc{x})} \sim N(0,1) \quad T = \frac{\wh{\mu}_{y \mid \vc{x}} - \mu_{y \mid \vc{x}}}{s_{SF} A(\vc{x})}  \sim t_{n - p}}
\end{align*}
\uncover<7->{\item A test statistic for testing $H_0:  \mu_{y \mid \vc{x}} = \#$ is:}
\begin{align*}
\uncover<8->{ \frac{{\wh{\mu}_{y \mid \vc{x}} - \#}}{s_{SF} A(\vc{x})}}
\end{align*}
\uncover<8->{which has a $t_{n - p}$ distribution under $H_0$.}
\uncover<9->{\item A 2-sided, $1 - \alpha$ confidence interval for $\mu_{y \mid \vc{x}}$ in compact form is $\wh{\mu}_{y \mid \vc{x}} \pm t_{n - p, 1 - \alpha/2} \cdot s_{SF} \cdot A(\vc{x})$.}
\uncover<10->{\item Note: $s_{SF} A(\vc{x})$ = $\wh{SD}(\wh{\mu}_{y \mid \vc{x}})$, which you can get directly from JMP output.}
\end{itemize}
\end{frame}
...
\end{align*}
\begin{itemize}
\uncover<5->{\item We're 95\% confident that when the air flow is 62, the temperature is 23 degrees, and the adjusted percentage of circulating acid is 87, the true mean stack loss is anywhere between 18.346 and 20.151 units.}
\end{itemize}
\end{frame}
